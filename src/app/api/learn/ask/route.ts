...
export async function POST(req: Request) {
  ...
  const asksAboutSmallGoals = /(?:目標|課題).{0,16}(?:小さ|細か)|(?:小さく|細かく).{0,16}(?:目標|課題)/.test(question);
  const focusInstruction = asksAboutControl
    ? "この質問には、最初に『いいえ。催眠は人の意思を奪って操ることではありません』と明答し、現在の事件でこれから命令と本人の反応を区別して確かめる、と説明する。"
    : asksAboutSmallGoals
      ? "この質問には違いを直接答える。目標を小さくするのは主に課題の大きさを変える方法。今回の催眠は、その前に『本当は全部を今すぐやらなければ』という自己暗示と、失敗を見張る注意から目を外す方法。その後で今できる感覚や動作を暗示に使う。両者は一緒に使えるが同じではない、と平易に説明する。"
...
- 「目標を小さくすることと何が違うか」と聞かれたら、手順を繰り返さない。課題の大きさを変えることと、失敗を見張る自己暗示・注意の向きを先に変えることの違いを対比する。
