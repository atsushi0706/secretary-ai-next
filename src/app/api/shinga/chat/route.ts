...
${memBlock}` : system;
        ...
        if (!greet) {
          ...
          if (mode === "money" && progress.moneyStep) {
            ...
            // 「もう出ない」を本人が何回言ったか（数えるのはこちら。3回で出し切ったとみなす）
            const noMore = sessionHistory.filter((m) => m.role === "user"
              && /(もう|これ以上)?(出ない|でない|ないです|ありません|思いつかない|浮かばない)/.test(m.content)).length;
            if (mStep <= 1 && noMore > 0) {
              lines.push(`  本人の「もう出ない」は、ここまで **${noMore}回**。`
                + (noMore >= 3 ? `3回に達した。出し切ったとみなして、金額に当てる（感覚的にいける？）へ進んでいい。`
                                : `まだ${3 - noMore}回ぶんは、角度を変えて出させる。`));
            }
            if (mStep <= 1) {
              lines.push(`  **まだ1段目＝資産を出す場。** 一つを掘らず、「ほかには？」で出し尽くす。`
                + `本人が思いつくのはスキルだけ。**まだ聞いていない方向**（相手に起こせる変化・見方・`
                + `人生経験・人柄・声・見た目・テンポ・年齢・声かけの癖・つながり・組み合わせ）を一つずつ当てにいく。`
                + `「もう出ない」が3回出るまで、または方向を回りきるまで出させる。「足りてる？」はそのあと。苦しそうなら金額を下げる提案をしていい。`
                + `本人が「足りている」と感じ、宣言（私は…すでにこれだけ持っています）を言い終えるまで、`
        ...
        if (greet) {
          const openLine = customWork
            ...
            ? `（「内なる子の神殿」で、${PARTS[partColor].defense.name}（${PARTS[partColor].defense.title}）のワークをいま始める。過去の別の話は持ち出さない。**しくみの説明は画面がもう見せたので繰り返さない**。前置きも断り書きも要らない。【1】①の問い「いま出てきてるのは、どんな感情？」だけを、1つ投げかけて）`
            : mode === "shadow"
...
JSONだけで返す：
{"corePower":"取り戻した力(20字以内・本人の言葉ベース)","ownership":"許可の一文(「私は…してよい」の形・40字以内)","otherResp":"相手に返す責任(30字以内)","boundary":"自分に許すこと(40字以内。境界線ではない。「◯◯してもいい」の形)","action24h":"明日それを自分に許す場面(30字以内)","before":数字かnull,"after":数字かnull}
