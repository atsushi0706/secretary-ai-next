...
const GEMINI_MODELS = ["gemini-3.5-flash", "gemini-3.1-flash-lite", "gemini-2.5-flash-lite", "gemini-2.5-flash"];
...
async function sttGemini(key: string, b64: string, mime: string): Promise<{ raw: string; polished: string } | null> {
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (geminiKey) {
      const g = await sttGemini(geminiKey, buf.toString("base64"), mime);
      if (g) {
    ...
    return json({ error: "文字化に失敗しました。少し待ってもう一度試してね。" }, 502);
  } catch (e: any) {
