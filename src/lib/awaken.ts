...
export type AbilityKey = "image" | "insight" | "grounding" | "locked1" | "locked2" | "locked3";
...
// 六角形のうち解放は3つ。残り3つは鍵のまま（設計が決まったら開ける）
...
export async function computeAbilities(userId: string): Promise<Ability[]> {
  ...
  const acc: Record<string, number> = { image: 0, insight: 0, grounding: 0 };
