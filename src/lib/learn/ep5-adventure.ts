...
export const EP5_ADVENTURE = defineAdventureScenario({
  ...
  nodes: [
    ...
    { kind: "recall", id: "ep5-memory", scene: "混乱を止める", purpose: "memory", title: "空の保管庫を見た時点で、確実に分かることは？", prompt: "リンク「僕の推測じゃなくて、淳が今見て分かることを一つ教えて。」", placeholder: "今見て確かめられる事実を一つ書く", helper: "『なぜ』ではなく、見えるものだけで大丈夫です。", storeAs: "ep5Memory", skipLabel: "まだ言葉にできない", skipValue: "保管庫にカードがない", replyFallback: "まず確実なのは、保管庫にカードがないことだね。意味は後で考えよう。" },
    ...
    { kind: "deduction", id: "ep5-deduction-1", scene: "第一推理", title: "ミオの映像を見た時点で、確定した行動はどれ？", prompt: "見た事実と、まだ証拠のない物語を分けてください。", questionBasis: { subject: "消えたカードとミオ", before: "映像が途中で切れた直後", asks: "現時点で確実に言えること", goal: "混乱を結論で埋めず、確認可能な事実へ注意を戻す" }, options: [
    ...
    { kind: "apply", id: "ep5-apply", scene: "リンクの混乱", title: "カードが消え、ミオもいない直後。混乱するリンクへ最初に何と言う？", prompt: "結論を押し付けず、現在の事実と選べる一手へ注意を戻します。", questionBasis: { subject: "ミオへの考えで頭がいっぱいのリンク", before: "裏切り、追え、信じるなという言葉が続く時", asks: "現在へ再定位する最初の一言", goal: "混乱が残っていても確認可能な次の行動を選べるようにする" }, storeAs: "ep5Move", options: [
