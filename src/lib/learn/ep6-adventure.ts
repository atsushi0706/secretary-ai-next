...
export const EP6_ADVENTURE = defineAdventureScenario({
  ...
  nodes: [
    ...
    { kind: "recall", id: "ep6-memory", scene: "言葉を読み直す", purpose: "memory", title: "今の一文に、実際に書いてあったことは？", prompt: "リンク「僕が足した意味は外して、画面の文字だけを教えて。」", placeholder: "画面に書かれた言葉だけを書く", helper: "『きっと』『つまり』は一度外してください。", storeAs: "ep6Memory", skipLabel: "文字だけ見る", skipValue: "カードを取り戻したいなら旧校舎へ", replyFallback: "書かれているのは、カードを取り戻したいなら旧校舎へ、だけだね。" },
    ...
    { kind: "guided-investigation", id: "ep6-guided", scene: "送信記録を追う", title: "言葉・場所・映像を一つずつ分ける", steps: [
      { id: "ep6-guide-1", linkPrompt: "まず画面の一文。文字にある条件と、文字にない条件を分けよう。", actionLabel: "1つ目の証拠を見る", evidenceId: "message", linkComment: "旧校舎へは書いてある。でも、一人で来いとは書いていない。", reflectionPrompt: "この一文だけでは決められないことは？", placeholder: "文字にないことを一つ書く", helper: "人数、時間、送り主を見てください。", storeAs: "ep6Observe1", skipLabel: "まだ分からない", skipValue: "誰といつ行くかは決められない", replyFallback: "人数も時間も、文字だけでは決められないね。", linkResponse: "{{ep6Observe1Reply}} 次は、位置情報が何を示すかだけ見よう。" },
