...
export const EP7_ADVENTURE = defineAdventureScenario({
  ...
  nodes: [
    ...
    { kind: "recall", id: "ep7-memory", scene: "選ばされる扉", purpose: "memory", title: "二つを選んだ後、どちらでも起きることは？", prompt: "リンク「違う部分じゃなく、同じ部分を教えて。」", placeholder: "二択に共通することを書く", helper: "誰が、何人で、何をするかを見てください。", storeAs: "ep7Memory", skipLabel: "共通点を見る", skipValue: "どちらも一人で扉へ入る", replyFallback: "時間は違っても、どちらも一人で入ることになるね。" },
    ...
    { kind: "guided-investigation", id: "ep7-guided", scene: "第三の道", title: "二択の外にある選択を探す", steps: [
      { id: "ep7-guide-1", linkPrompt: "表示された二択に共通する前提を、もう一度画面で確かめよう。", actionLabel: "1つ目の証拠を見る", evidenceId: "door-choice", linkComment: "どちらにも『一人で入る』が入っている。ここは選べていない。", reflectionPrompt: "この二択で、選べないように見えることは？", placeholder: "二択から消えた選択を書く", helper: "入らない、二人で入る、後で決めるを考えてください。", storeAs: "ep7Observe1", skipLabel: "まだ分からない", skipValue: "一人で入らない選択が見えない", replyFallback: "入らないことも、二人で入ることも画面にはないね。", linkResponse: "{{ep7Observe1Reply}} 表示を押す前に、扉そのものを調べよう。" },
