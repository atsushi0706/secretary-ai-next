 ...
 *   classroom  … 教室。先生とリンクの掛け合い＋スライド。質問チケットが使える
...
export type ExpStep =
  ...
  | { kind: "choice"; q: string; options: { label: string; then: ExpStep[] }[] }
...
export type Part =
  | { kind: "manga"; title: string; pages: MangaPage[]; /** 最後に出す文 */ close?: string[] }
