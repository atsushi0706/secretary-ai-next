...
- **ただの用事を並べない。** 選ぶのは「**その人の理想に通じているもの**」。
  買い物・返信・事務のような、こなすだけのものは入れない。
  今日の話の中で、その人が「こうなりたい」「こうしたい」と言っていたほうへ、
  半歩でも近づくものを選ぶ。小さくていい（「◯◯を15分だけ調べる」でも、
  それが理想の方向を向いていればいい）。
...
# 出す形（JSONだけ。前後に何も書かない）
{
  ...
  "tomorrow": ["明日やること（20字以内）。**理想に通じているもの**だけ。最大3つ"],
  "emotion": "明日の1日を、どんな感じで終えたいか。1語（10字以内。例：満ちている／軽い／誇らしい）",
